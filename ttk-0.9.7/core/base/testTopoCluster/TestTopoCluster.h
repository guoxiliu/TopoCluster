...
template <class dataType> int ttk::TestTopoCluster::execute() const{
  ...
  //   SimplexId neighborNum = triangulation_->getCellNeighborNumber(cellId);
  //   for(SimplexId j = 0; j < neighborNum; j++){
  //     int result1 = triangulation_->getCellNeighbor(cellId, j, neighborId);
  //     if(result1){
  //       std::cout << "[TestTopoCluster] cellId " << cellId << " Something wrong in getCellNeighbor()! Error code: " << result1 << "\n";
  //     }
  //   }
  // }
  // std::cout << "[TestTopoCluster] Time usage for CN: " << t.getElapsedTime() << " s.\n";
