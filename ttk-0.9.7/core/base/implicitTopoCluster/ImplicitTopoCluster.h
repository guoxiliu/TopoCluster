/// \ingroup base
/// \class ttk::ImplicitTopoCluster 
/// \author Guoxi Liu <guoxil@g.clemson.edu>
/// \date Nov. 2019.
///
/// \brief TTK %ImplicitTopoCluster processing package.
///
/// %ImplicitTopoCluster is a TTK processing package that takes a scalar field on the input 
/// and produces a scalar field on the output.
///
/// \sa ttk::Triangulation
/// \sa ttkImplicitTopoCluster.cpp %for a usage example.
...
namespace ttk{
  ...
  public:
    ...
    friend class ImplicitTopoCluster;
  };

  
  class ImplicitTopoCluster : public AbstractTriangulation{
    ...
    public:
      ...
      int setInputCells(const SimplexId &cellNumber,
        ...
        cout << "Node number: " << nodeNumber_ << endl;
        return 0;
      }
      ...
      const vector<pair_int > *getEdges(){
        ...
        for(SimplexId nid = 1; nid <= nodeNumber_; nid++){
          ...
          buildInternalEdgeMap(nid, &localInternalEdges, &localInternalEdgeMap, nullptr);
      ...
      int getEdgeVertex(const SimplexId &edgeId, 
        ...
        if(exnode->internalEdgeList_ == nullptr){
          ...
          buildInternalEdgeMap(nid, exnode->internalEdgeList_, exnode->internalEdgeMap_, nullptr);
        }
        ...
        if(getDimensionality() == 2){
          ...
          for(SimplexId nid = 1; nid <= nodeNumber_; nid++){
            ...
            if(exnode->internalTriangleList_ == nullptr){
              ...
              buildInternalTriangleMap(nid, exnode->internalTriangleList_, exnode->internalTriangleMap_, nullptr);
            }
      ...
      int preprocessBoundaryEdges(){
        if(getDimensionality() == 2 || getDimensionality() == 3){
          ...
          msg << "[ImplicitTopoCluster] Unsupported dimension for boundary "
      ...
      int preprocessEdges(){
        ...
        if(!hasPreprocessedEdges_){
          ...
          cout << "[ImplicitTopoCluster] Edges processed in " << t.getElapsedTime() << " s.\n";
        }
      ...
      int preprocessEdgeLinks(){
        if(getDimensionality() == 2 || getDimensionality() == 3){
          ...
          std::stringstream msg;
          msg 
            << "[ImplicitTopoCluster] Unsupported dimension for edge link "
            << "preprocessing." << std::endl;
          dMsg(std::cerr, msg.str(), infoMsg);
          return -1;
        }
      ...
      int preprocessTriangles(){
        ...
        if(!hasPreprocessedTriangles_){
          ...
          hasPreprocessedTriangles_ = true;
          cout << "[ImplicitTopoCluster] Triangles processed in " << t.getElapsedTime() << " s.\n";
        }
      ...
      int preprocessVertexLinks(){
        if(getDimensionality() == 2){
          ...
          // unsupported dimension
          std::stringstream msg;
          msg << "[ImplicitTopoCluster] Unsupported dimension for vertex"
    ...
    protected:
      ...
      int buildInternalEdgeMap(SimplexId nodeId, 
        ...
        if(internalEdgeMap){
          // if the edge map has been computed before and only request the edge list
          if(!internalEdgeMap->empty() && internalEdgeList){
            internalEdgeList->resize(internalEdgeMap->size());
            for(auto iter = internalEdgeMap->begin(); iter != internalEdgeMap->end(); iter++){
      ...
      int buildExternalEdgeMap(ImplicitCluster * const nodePtr) const{
          ...
          for(SimplexId j = 0; j < verticesPerCell-1; j++){
            for(SimplexId k = j+1; k < verticesPerCell; k++){
              ...
              if(edgeIds.first <= vertexIntervals_[nodePtr->nid-1] && edgeIds.second > vertexIntervals_[nodePtr->nid-1] && edgeIds.second <= vertexIntervals_[nodePtr->nid]){
                SimplexId nid = vertexIndices_[edgeIds.first];
        ...
        for(iter = edgeNodes.begin(); iter != edgeNodes.end(); iter++){
          ...
          if(!exnode){
            boost::unordered_map<pair_int, SimplexId> localInternalEdgeMap;
            buildInternalEdgeMap(iter->first, nullptr, &localInternalEdgeMap, nullptr);
            for(pair_int edgePair : iter->second){
              (*(nodePtr->externalEdgeMap_))[edgePair] = localInternalEdgeMap.at(edgePair)+edgeIntervals_[iter->first-1];
            }
          }
          else{
            if(exnode->internalEdgeMap_ == nullptr){
              exnode->internalEdgeMap_ = new boost::unordered_map<pair_int, SimplexId>();
              buildInternalEdgeMap(iter->first, nullptr, exnode->internalEdgeMap_, nullptr);
            }
            for(pair_int edgePair : iter->second){
              (*(nodePtr->externalEdgeMap_))[edgePair] = exnode->internalEdgeMap_->at(edgePair)+edgeIntervals_[iter->first-1];
            }
          }
        }
      ...
      int buildInternalTriangleMap(SimplexId nodeId, 
        ...
        if(internalTriangleMap){
          // if the triangle map has been computed before and only request the triangle list
          if(!internalTriangleMap->empty() && internalTriangleList){
            internalTriangleList->resize(internalTriangleMap->size());
            for(auto iter = internalTriangleMap->begin(); iter != internalTriangleMap->end(); iter++){
            ...
            for(SimplexId k = j+1; k < verticesPerCell-1; k++){
              for(SimplexId l = k+1; l < verticesPerCell; l++){
                ...
                if(triangleMap->find(triangleIds) == triangleMap->end()){
                  ...
                  triangleStars->at(triangleMap->at(triangleIds)-1).push_back(cid);
                }
              }
            }
          }
      ...
      int buildExternalTriangleMap(ImplicitCluster * const nodePtr) const{
              ...
              for(SimplexId k = j+1; k < verticesPerCell-1; k++){
                for(SimplexId l = k+1; l < verticesPerCell; l++){
                  ...
                  if(triangleIds[1] > vertexIntervals_[nodePtr->nid-1] && triangleIds[1] <= vertexIntervals_[nodePtr->nid]){
                    ...
                    triangleNodes[nodeNum].push_back(triangleIds);
                  }
        ...
        for(iter = triangleNodes.begin(); iter != triangleNodes.end(); iter++){
          ...
          if(!exnode){
            ...
            buildInternalTriangleMap(iter->first, nullptr, &localInternalTriangleMap, nullptr);
            for(vector<SimplexId> triangleVec : iter->second){
              (*(nodePtr->externalTriangleMap_))[triangleVec] = localInternalTriangleMap.at(triangleVec)+triangleIntervals_[iter->first-1];
            }
          }
          else{
            if(exnode->internalTriangleMap_ == nullptr){
              exnode->internalTriangleMap_ = new boost::unordered_map<vector<SimplexId>, SimplexId>();
              buildInternalTriangleMap(iter->first, nullptr, exnode->internalTriangleMap_, nullptr);
            }
            for(vector<SimplexId> triangleVec : iter->second){
              (*(nodePtr->externalTriangleMap_))[triangleVec] = exnode->internalTriangleMap_->at(triangleVec)+triangleIntervals_[iter->first-1];
            }
          }
        }
      ...
      int getCellEdges(ImplicitCluster * const nodePtr) const{
        ...
        if(nodePtr->internalEdgeMap_ == nullptr){
          ...
          buildInternalEdgeMap(nodePtr->nid, nullptr, nodePtr->internalEdgeMap_, nullptr);
        }
        ...
        for(SimplexId i = cellIntervals_[nodePtr->nid-1]+1; i <= cellIntervals_[nodePtr->nid]; i++){
          ...
          for(SimplexId k = 1; k < cellArray_[0]; k++){
            ...
            (*(nodePtr->cellEdges_))[i-cellIntervals_[nodePtr->nid-1]-1][cnt++] = 
              nodePtr->internalEdgeMap_->at(edgePair)+edgeIntervals_[nodePtr->nid-1];
          }
          for(SimplexId j = 1; j < cellArray_[0]-1; j++){
            for(SimplexId k = j+1; k < cellArray_[0]; k++){
              ...
              // group the external edges by node id
                ...
                vector<SimplexId> edgeTuple{i, cnt++, edgePair.first, edgePair.second};
        ...
        for(auto iter = edgeNodes.begin(); iter != edgeNodes.end(); iter++){
          ...
          if(!exnode){
            ...
            for(vector<SimplexId> edgeTuple : iter->second){
              ...
              (*nodePtr->cellEdges_)[edgeTuple[0]-cellIntervals_[nodePtr->nid-1]-1][edgeTuple[1]] = 
                localInternalEdgeMap.at(edgePair)+edgeIntervals_[iter->first-1];
            }
          }
          else{
            if(exnode->internalEdgeMap_ == nullptr){
            ...
            for(vector<SimplexId> edgeTuple : iter->second){
              pair_int edgePair(edgeTuple[2], edgeTuple[3]);
              (*nodePtr->cellEdges_)[edgeTuple[0]-cellIntervals_[nodePtr->nid-1]-1][edgeTuple[1]] = 
                exnode->internalEdgeMap_->at(edgePair)+edgeIntervals_[iter->first-1];
            }
          }
        }
      ...
      int getCellTriangles(ImplicitCluster * const nodePtr) const{
        ...
        if(nodePtr->internalTriangleMap_ == nullptr){
          ...
          buildInternalTriangleMap(nodePtr->nid, nullptr, nodePtr->internalTriangleMap_, nullptr);
        }
        ...
        for(SimplexId i = cellIntervals_[nodePtr->nid-1]+1; i <= cellIntervals_[nodePtr->nid]; i++){
          ...
          for(SimplexId k = 1; k < cellArray_[0]-1; k++){
            ...
            for(SimplexId l = k+1; l < cellArray_[0]; l++){
              ...
              (*(nodePtr->cellTriangles_))[i-cellIntervals_[nodePtr->nid-1]-1][k+l-3] = 
                nodePtr->internalTriangleMap_->at(triangleVec)+triangleIntervals_[nodePtr->nid-1];
            }
          }
        ...
        for(auto iter = triangleNodes.begin(); iter != triangleNodes.end(); iter++){
          ...
          if(!exnode){
            ...
            for(vector<SimplexId> triangleVec : iter->second){
              ...
              (*(nodePtr->cellTriangles_))[triangleVec[0]-cellIntervals_[nodePtr->nid-1]-1].back() = 
                localInternalTriangleMap.at(triangle)+triangleIntervals_[iter->first-1];
            }
          }
          else{
            if(exnode->internalTriangleMap_ == nullptr){
            ...
            for(vector<SimplexId> triangleVec : iter->second){
              vector<SimplexId> triangle(triangleVec.begin()+1, triangleVec.end());
              (*(nodePtr->cellTriangles_))[triangleVec[0]-cellIntervals_[nodePtr->nid-1]-1].back() = 
                exnode->internalTriangleMap_->at(triangle)+triangleIntervals_[iter->first-1];
            }
          }
        }
      ...
      int getEdgeLinks(ImplicitCluster * const nodePtr) const{
        ...
        if(getDimensionality() == 2){
          if(nodePtr->edgeStars_ == nullptr){
            ...
            buildInternalEdgeMap(nodePtr->nid, nullptr, nodePtr->internalEdgeMap_, nodePtr->edgeStars_);
          }
          boost::unordered_map<pair_int, SimplexId>::const_iterator iter;
          for(iter = nodePtr->internalEdgeMap_->begin(); iter != nodePtr->internalEdgeMap_->end(); iter++){
          ...
          for(SimplexId cid = 0; cid < cellIntervals_[nodePtr->nid]-cellIntervals_[nodePtr->nid-1]; cid++){
            ...
            for(SimplexId j = 1; j < 4; j++){
              ...
              (*(nodePtr->edgeLinks_))[nodePtr->internalEdgeMap_->at(edgePair)-1].push_back((*(nodePtr->cellEdges_))[cid][6-j]);
            }
            if(cellArray_[cellId+2] <= vertexIntervals_[nodePtr->nid]){
              edgePair.first = cellArray_[cellId+2];
              for(int j = 2; j < 4; j++){
                edgePair.second = cellArray_[cellId+j+1];
                (*(nodePtr->edgeLinks_))[nodePtr->internalEdgeMap_->at(edgePair)-1].push_back((*(nodePtr->cellEdges_))[cid][4-j]);
              }
              if(cellArray_[cellId+3] <= vertexIntervals_[nodePtr->nid]){
                edgePair = pair_int(cellArray_[cellId+3], cellArray_[cellId+4]);
                (*(nodePtr->edgeLinks_))[nodePtr->internalEdgeMap_->at(edgePair)-1].push_back((*(nodePtr->cellEdges_))[cid][0]);
              }
            }
          }
            ...
            for(SimplexId j = 1; j < 4; j++){
              for(SimplexId k = j+1; k < 5; k++){
                ...
                if(edgeIds.first > vertexIntervals_[nodePtr->nid-1] && edgeIds.first <= vertexIntervals_[nodePtr->nid]){
                  ...
                  for(int i = 1; i < 5; i++){
                    if(cellArray_[cellId+i] != edgeIds.first && cellArray_[cellId+i] != edgeIds.second){
                      if(otherEdge.first == -1){
                        ...
                        cerr << "[ImplicitTopoCluster] More than two other vertices are found in the edge!\n";
                      }
                    }
                  }
                  ...
                  if(nodeMaps.find(nodeId) == nodeMaps.end()){
                    ImplicitCluster *newNode = new ImplicitCluster(nodeId);
                    newNode->internalEdgeMap_ = new boost::unordered_map<pair_int, SimplexId>();
                    buildInternalEdgeMap(nodeId, nullptr, newNode->internalEdgeMap_, nullptr);
                    nodeMaps[nodeId] = newNode;
                  }
                  (*(nodePtr->edgeLinks_))[nodePtr->internalEdgeMap_->at(edgeIds)-1].push_back(
      ...
      int getEdgeTriangles(ImplicitCluster * const nodePtr) const{
        ...
        for(iter = nodePtr->internalTriangleMap_->begin(); iter != nodePtr->internalTriangleMap_->end(); iter++){
          pair_int edge1 = pair_int(iter->first[0], iter->first[1]);
          pair_int edge2 = pair_int(iter->first[0], iter->first[2]);

          (*nodePtr->edgeTriangles_)[nodePtr->internalEdgeMap_->at(edge1)-1].push_back(iter->second + triangleIntervals_[nodePtr->nid-1]);
          (*nodePtr->edgeTriangles_)[nodePtr->internalEdgeMap_->at(edge2)-1].push_back(iter->second + triangleIntervals_[nodePtr->nid-1]);

          if(iter->first[1] <= vertexIntervals_[nodePtr->nid]){
            edge1 = pair_int(iter->first[1], iter->first[2]);
            (*nodePtr->edgeTriangles_)[nodePtr->internalEdgeMap_->at(edge1)-1].push_back(iter->second + triangleIntervals_[nodePtr->nid-1]);
          }
        }
        ...
        for(iter = nodePtr->externalTriangleMap_->begin(); iter != nodePtr->externalTriangleMap_->end(); iter++){
          ...
          if(edge.first > vertexIntervals_[nodePtr->nid-1] && edge.first <= vertexIntervals_[nodePtr->nid]){
            (*nodePtr->edgeTriangles_)[nodePtr->internalEdgeMap_->at(edge)-1].push_back(iter->second);
          }
        }
      ...
      int getTriangleEdges(ImplicitCluster * const nodePtr) const{
        ...
        for(auto iter = nodePtr->internalTriangleMap_->begin(); iter != nodePtr->internalTriangleMap_->end(); iter++){
          // since the first vertex of the triangle is in the node ...
          pair_int edgePair(iter->first[0], iter->first[1]);
          (*nodePtr->triangleEdges_)[iter->second-1][0] = nodePtr->internalEdgeMap_->at(edgePair)+edgeIntervals_[nodePtr->nid-1];
          edgePair.second = iter->first[2];
          (*nodePtr->triangleEdges_)[iter->second-1][1] = nodePtr->internalEdgeMap_->at(edgePair)+edgeIntervals_[nodePtr->nid-1];
          edgePair.first = iter->first[1];
          if(edgePair.first > vertexIntervals_[nodePtr->nid-1] && edgePair.first <= vertexIntervals_[nodePtr->nid]){
            (*nodePtr->triangleEdges_)[iter->second-1][2] = nodePtr->internalEdgeMap_->at(edgePair)+edgeIntervals_[nodePtr->nid-1];
          }else{
            vector<SimplexId> edgeTuple{iter->second-1, edgePair.first, edgePair.second};
            SimplexId nodeNum = vertexIndices_[edgePair.first];
            edgeNodes[nodeNum].push_back(edgeTuple);
          }
        }
        ...
        for(auto iter = edgeNodes.begin(); iter != edgeNodes.end(); iter++){
          ...
          if(!exnode){
            ...
            for(vector<SimplexId> edgeTuple : iter->second){
              ...
              (*nodePtr->triangleEdges_)[edgeTuple[0]][2] = localInternalEdgeMap.at(edgePair)+edgeIntervals_[iter->first-1];
            }
          }
          else{
            if(exnode->internalEdgeMap_ == nullptr){
            ...
            for(vector<SimplexId> edgeTuple : iter->second){
              pair_int edgePair(edgeTuple[1], edgeTuple[2]);
              (*nodePtr->triangleEdges_)[edgeTuple[0]][2] = exnode->internalEdgeMap_->at(edgePair)+edgeIntervals_[iter->first-1];
            }
          }
        }
      ...
      int getTriangleLinks(ImplicitCluster * const nodePtr) const{
        ...
        if(nodePtr->triangleStars_ == nullptr){
          ...
          buildInternalTriangleMap(nodePtr->nid, nullptr, nodePtr->internalTriangleMap_, nodePtr->triangleStars_);
        }
      ...
      int getVertexEdges(ImplicitCluster * const nodePtr) const{
        ...
        for(iter = nodePtr->internalEdgeMap_->begin(); iter != nodePtr->internalEdgeMap_->end(); iter++){
          (*(nodePtr->vertexEdges_))[iter->first.first-vertexIntervals_[nodePtr->nid-1]-1].push_back(edgeIntervals_[nodePtr->nid-1]+iter->second);
          if(iter->first.second <= vertexIntervals_[nodePtr->nid]){
            (*(nodePtr->vertexEdges_))[iter->first.second-vertexIntervals_[nodePtr->nid-1]-1].push_back(edgeIntervals_[nodePtr->nid-1]+iter->second);
          }
        }
        ...
        if(getDimensionality() == 2){
          ...
          for(SimplexId cid = cellIntervals_[nodePtr->nid-1]+1; cid <= cellIntervals_[nodePtr->nid]; cid++){
            ...
            (*(nodePtr->vertexLinks_))[cellArray_[cellId+1]-vertexIntervals_[nodePtr->nid-1]-1].push_back(
              nodeMaps[nodeId]->internalEdgeMap_->at(edgePair) + edgeIntervals_[nodeId-1]);
            if(cellArray_[cellId+2] <= vertexIntervals_[nodePtr->nid]){
          ...
          for(SimplexId cid = cellIntervals_[nodePtr->nid-1]+1; cid <= cellIntervals_[nodePtr->nid]; cid++){
            ...
            if(nodeMaps.find(nodeId) == nodeMaps.end()){
              ImplicitCluster *newNode = new ImplicitCluster(nodeId);
              newNode->internalTriangleMap_ = new boost::unordered_map<vector<SimplexId>, SimplexId>();
              buildInternalTriangleMap(nodeId, nullptr, newNode->internalTriangleMap_, nullptr);
              nodeMaps[nodeId] = newNode;
            }
            (*(nodePtr->vertexLinks_))[cellArray_[cellId+1]-vertexIntervals_[nodePtr->nid-1]-1].push_back(
              nodeMaps[nodeId]->internalTriangleMap_->at(triangleVec) + triangleIntervals_[nodeId-1]);
            // v2: (v1, v3, v4)
            if(cellArray_[cellId+2] <= vertexIntervals_[nodePtr->nid]){
              triangleVec[0] = cellArray_[cellId+1];
              (*(nodePtr->vertexLinks_))[cellArray_[cellId+2]-vertexIntervals_[nodePtr->nid-1]-1].push_back(
      ...
      int getVertexNeighbors(ImplicitCluster * const nodePtr) const{
        ...
        if(nodePtr->internalEdgeMap_ == nullptr){
          // no need to build the internal edge list
          SimplexId v1, v2;
          ...
          for(SimplexId cid = cellIntervals_[nodePtr->nid-1]+1; cid <= cellIntervals_[nodePtr->nid]; cid++){
            ...
            for(SimplexId j = 0; j < cellArray_[0]-1; j++){
              v1 = cellArray_[cellId+j+1];
              if(v1 <= vertexIntervals_[nodePtr->nid]){
                for(SimplexId k = j+1; k < cellArray_[0]; k++){
                  v2 = cellArray_[cellId+k+1];
                  vertexNeighborSet[v1-vertexIntervals_[nodePtr->nid-1]-1].insert(v2);
                  if(v2 <= vertexIntervals_[nodePtr->nid]){
                    vertexNeighborSet[v2-vertexIntervals_[nodePtr->nid-1]-1].insert(v1);
                  }
                }
              }
            }
          ...
          for(SimplexId cid : externalCells_[nodePtr->nid]){
            ...
            for(SimplexId j = 0; j < cellArray_[0]-1; j++){
              for(SimplexId k = j+1; k < cellArray_[0]; k++){
                v1 = cellArray_[cellId+j+1];
                v2 = cellArray_[cellId+k+1];
                if(v1 > vertexIntervals_[nodePtr->nid-1] && v1 <= vertexIntervals_[nodePtr->nid])
                  vertexNeighborSet[v1-vertexIntervals_[nodePtr->nid-1]-1].insert(v2);
                if(v2 > vertexIntervals_[nodePtr->nid-1] && v2 <= vertexIntervals_[nodePtr->nid])
                  vertexNeighborSet[v2-vertexIntervals_[nodePtr->nid-1]-1].insert(v1);
              }
            }
          }
          ...
          for(SimplexId i = 0; i < localVertexNum; i++){
            (*(nodePtr->vertexNeighbors_))[i] = vector<SimplexId>(vertexNeighborSet[i].begin(), vertexNeighborSet[i].end());
          }
        }
      ...
      int getBoundaryCells(ImplicitCluster * const nodePtr, const SimplexId dim = 2) const{
        ...
        if(getDimensionality() == 2){
          ...
          if(dim == 0 && nodePtr->boundaryVertices_ == nullptr){
            ...
            // internal edges
            for(auto iter = nodePtr->internalEdgeMap_->begin(); iter != nodePtr->internalEdgeMap_->end(); iter++){
          ...
          if(dim == 1 && nodePtr->boundaryEdges_ == nullptr){
            ...
            for(auto iter = nodePtr->internalTriangleMap_->begin(); iter != nodePtr->internalTriangleMap_->end(); iter++){
              if((*(nodePtr->boundaryTriangles_))[iter->second-1]){
                pair_int edgePair(iter->first[0], iter->first[1]);
                (*(nodePtr->boundaryEdges_))[nodePtr->internalEdgeMap_->at(edgePair)-1] = true;
                edgePair.second = iter->first[2];
                (*(nodePtr->boundaryEdges_))[nodePtr->internalEdgeMap_->at(edgePair)-1] = true;
                if(iter->first[1] <= vertexIntervals_[nodePtr->nid]){
                  edgePair.first = iter->first[1];
      ...
      mutable vector<boost::unordered_map<SimplexId, list<ImplicitCluster*>::iterator>> cacheMaps_;
